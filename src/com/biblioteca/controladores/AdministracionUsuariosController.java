...
public class AdministracionUsuariosController {
    ...
    public void manejarBotonAgregar() {
        if (!vista.isNuevoModo() && !vista.isEditMode()) {
            ...
            actualizarPanelBotones(vista.getBtnAgregar(), vista.getBtnCancelar());
        ...
        if (!vista.isEditMode() && !vista.isNuevoModo()) {
            ...
            if (selectedRow >= 0) {
                ...
                if (confirmEditar == JOptionPane.YES_OPTION) {
                    ...
                    vista.getBtnActualizar().setText("Guardar Cambios");
                    vista.setEditMode(true);

                    vista.getBtnAgregar().setVisible(false);
                    vista.getBtnEliminar().setEnabled(false);
                    actualizarPanelBotones(vista.getBtnActualizar(), vista.getBtnCancelar());
                }
            } else {
    ...
    public void agregarUsuario() {
        try {
            ...
            if (agregado) {
                ...
                actualizarPanelBotones(vista.getBtnAgregar(), vista.getBtnActualizar());
    ...
    // Método auxiliar para actualizar los botones en el panel
    private void actualizarPanelBotones(JButton boton1, JButton boton2) {
        vista.getPanelBotones().removeAll();
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(0, 20, 0, 20);
        gbc.gridy = 0;

        gbc.gridx = 0;
        vista.getPanelBotones().add(boton1, gbc);

        gbc.gridx = 1;
        vista.getPanelBotones().add(boton2, gbc);
